...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
volatile int8_t acc[5];

void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {

	uint8_t auto_increment = 0b10000000;
	uint8_t sub_address = OUT_X | auto_increment;
...
void HAL_I2C_MasterRxCpltCallback(I2C_HandleTypeDef *hi2c){
	float acc_x_g = acc[0] / 64.0;
	float acc_y_g = acc[2] / 64.0;
	float acc_z_g = acc[4] / 64.0;

	len = snprintf(str,sizeof(str), "X: %+.2f g\t Y: %+.2f g\t Z: %+.2f g\r\n", acc_x_g,acc_y_g,acc_z_g);
